...
class ReaderContainerViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
    ...
    private var currentLayoutSpec: ReaderLayoutSpec {
        // 统一计算：所有地方都必须调用这个 Spec
        ReaderLayoutSpec(
            topInset: safeAreaTop + 20,
            bottomInset: safeAreaBottom + 40,
    ...
    func jumpToChapter(_ index: Int, startAtEnd: Bool = false) {
        ...
        loadChapterContent(at: index, resetOffset: true, startAtEnd: startAtEnd)
    }
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    if resetOffset {
                        ...
                        let target = startAtEnd ? max(0, self.pages.count - 1) : 0
                        self.updateHorizontalPage(to: target, animated: false, direction: startAtEnd ? .reverse : .forward)
    ...
    func pageViewController(_ pvc: UIPageViewController, didFinishAnimating f: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        if completed, let visibleVC = pvc.viewControllers?.first as? PageContentViewController {
            ...
            if visibleVC.chapterOffset == 0 {
                ...
                else { nextChapterStore = renderStore; nextChapterPages = pages; nextChapterPageInfos = pageInfos; renderStore = prevChapterStore; pages = prevChapterPages; pageInfos = prevChapterPageInfos; prevChapterStore = nil; prevChapterPages = [] }
    ...
    private func removeHTMLAndSVG(_ text: String) -> String {
        var res = text; let patterns = ["<svg[^>]*>.*?<\/svg>", "<img[^>]*>", "<[^>]+>"]; for p in patterns { if let regex = try? NSRegularExpression(pattern: p, options: [.caseInsensitive, .dotMatchesLineSeparators]) { res = regex.stringByReplacingMatches(in: res, options: [], range: NSRange(location: 0, length: res.utf16.count), withTemplate: "") } }
